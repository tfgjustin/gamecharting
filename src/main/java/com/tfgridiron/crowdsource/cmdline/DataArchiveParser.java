...
public class DataArchiveParser {
  ...
  final TeamIndexer teamIndexer;
  Season season = null;

  public DataArchiveParser(TeamIndexer teamIndexer) {
  ...
  public boolean hasGameData(String gameId) {
    ...
    return season.getGameTable().hasGame(gameId)
        && !season.getPlayTable().getPlays(gameId).isEmpty();
  }

  public boolean fetchRowData(String gameId, Integer playId, Map<String, String> outputColumns) {
    // For a given game/play, load the data we've found, storing the output in a mapping from "PBP"
    // sheet column header to the value for that column
    ...
    outputColumns.put("Away", teamIndexer.getShortName(game.getVisitingTeam()));
    ...
    outputColumns.put("Home", teamIndexer.getShortName(game.getHomeTeam()));
    ...
    outputColumns.put("Offense", teamIndexer.getShortName(offenseTeamCode));
    ...
    if (rushRow != null && passRow != null) {
      ...
      throw new RuntimeException("This should never happen, but it did on Game "
          + playKey.getGameCode() + ", play " + playKey.getPlayNum());
    }
    if (passRow != null) {
    ...
    if (rushRow != null) {
      outputColumns.put("RunnerIntendedReceiver",
          season.getPlayerTable().getPlayerName(rushRow.getPlayerCode()));
    } else if (passRow != null) {
      outputColumns.put("RunnerIntendedReceiver",
          season.getPlayerTable().getPlayerName(passRow.getReceiverPlayerCode()));
    } else {
      outputColumns.put("RunnerIntendedReceiver", "");
    }
    ...
    if (passRow != null && passRow.isAttempted()) {
      outputColumns.put("Complete", Boolean.toString(passRow.isComplete()));
    } else {
